...
export async function GET(request: Request) {
  ...
  let workspaceId = "";
  try {
    workspaceId = JSON.parse(Buffer.from(rawState, "base64url").toString("utf8")).workspaceId || "";
  ...
  const profileResponse = await fetch("https://www.googleapis.com/oauth2/v3/userinfo", { headers: { Authorization: `Bearer ${tokenData.access_token}` } }).catch(() => undefined);
  ...
  const connection: Connection = {
    ...
    provider: "google",
    status: "connected",
    label: profile.email || "Google Gmail + Drive",
